<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-gray-500 mt-1">Generate laporan stok dan transaksi</p>
        </div>
    ...
    <div class="card p-4 mb-6">
        <div class="flex flex-col lg:flex-row gap-4">
            <select wire:model.live="reportType" class="w-full lg:w-48 px-4 py-2.5 border border-gray-300 rounded-lg">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Satuan</th>
                        <th class="px-6 py-3 text-center text-xs font-semibold text-gray-600 uppercase">Stok</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-3 font-mono text-sm text-gray-600">{{ $item->kode_barang }}</td>
                            <td class="px-6 py-3 font-medium text-gray-900">{{ $item->nama_barang }}</td>
                            <td class="px-6 py-3 text-center text-gray-600">{{ $item->satuan?->nama_satuan }}</td>
                            <td class="px-6 py-3 text-center font-semibold">{{ $item->qty_barang }}</td>
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Oleh</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-3 font-medium">{{ $item->no_barang_masuk }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $item->tanggal->format('d M Y') }}</td>
                            <td class="px-6 py-3 text-center">{{ $item->total_items }}</td>
                            <td class="px-6 py-3 text-right font-semibold">Rp {{ number_format($item->total_value, 0, ',', '.') }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $item->createdUser?->name }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-6 py-12 text-center text-gray-500">Tidak ada data</td></tr>
                    @endforelse
        ...
        @elseif($reportType === 'keluar')
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-3 font-medium">{{ $item->no_barang_keluar }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $item->tanggal->format('d M Y') }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $item->order?->no_order ?? '-' }}</td>
                            <td class="px-6 py-3 text-center">{{ $item->total_items }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $item->createdUser?->name }}</td>
                        </tr>
                    @empty
        ...
        @elseif($reportType === 'order')
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-3 font-medium">{{ $item->no_order }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $item->tanggal->format('d M Y') }}</td>
                            <td class="px-6 py-3 text-gray-600">{{ $item->nama_user_request ?? $item->createdUser?->name }}</td>
                            <td class="px-6 py-3 text-center">{{ $item->total_items }}</td>
                            <td class="px-6 py-3 text-center">
