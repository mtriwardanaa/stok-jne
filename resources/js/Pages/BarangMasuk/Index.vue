    ...
    <AppLayout title="Barang Masuk">
        <div class="space-y-6">
            ...
            <div class="relative rounded-2xl p-6 bg-white border border-slate-200 shadow-sm">
                <div class="absolute top-0 right-0 -mr-10 -mt-10 w-40 h-40 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 rounded-full blur-3xl"></div>
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 relative z-10">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-xl font-bold text-slate-800">Barang Masuk</h2>
                            <p class="text-sm text-slate-500">Riwayat penerimaan barang ke gudang</p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="bm in barangMasuks.data" :key="bm.id" class="hover:bg-slate-50/80">
                            <td class="px-6 py-4 font-medium text-slate-900">{{ bm.no_barang_masuk }}</td>
                            <td class="px-6 py-4 text-sm text-slate-500">{{ formatDate(bm.tanggal) }}</td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-slate-600">{{ bm.created_user?.name || '-' }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="px-2 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-semibold">{{ bm.details?.length || 0 }} item</span>
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium text-slate-700">
                                Rp {{ formatCurrency(getTotalValue(bm.details || [])) }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <Link :href="`/barang-masuk/${bm.id}`" class="text-slate-400 hover:text-emerald-600">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                                </Link>
                            </td>
                        </tr>
                        <tr v-if="barangMasuks.data.length === 0">
                            <td colspan="6" class="px-6 py-12 text-center text-slate-500">Tidak ada data barang masuk</td>
                        </tr>
                    </tbody>
                </table>
            </div>
